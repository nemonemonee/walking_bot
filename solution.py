...
class SOLUTION:
    ...
    def Create_World(self):
        ...
        pyrosim.Send_Cube(name="Box0", pos=[-5,0,.2] , size=[5,10,.5])
        for i in range(1,10):
    ...
    def InitializeRandomVariables(self):
        self.length = 2 + numpy.random.randint(7)
        ...
        self.numMotorNeurons = 2 * (self.length - 1)
    ...
    def BuildBodyPlan(self):
        ...
        for i in range(2, self.length):
          ...
          if len(self.bodyPlan[pre_i]) >= 3: potentialParent.remove(pre_i)
    ...
    def ArrangementPlan(self):
        ...
        for i in range(len(self.bodyPlan)):
           for j in self.bodyPlan[i]:
             ...
             self.growDir[j] = d
             if d == 3: self.toBotm += self.size[j,2]
    ...
    def save(self):
        filename = "save" + str(self.myID) + ".soln"
